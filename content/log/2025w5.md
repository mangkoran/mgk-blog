+++
title = "Week 5 2025"
date = 2025-02-01
draft = true
[taxonomies]
tags = ["log"]
+++

## Work

- Dev env migration
  - Route53 zone, IAM user role for k8s External DNS done
  - VPC done
  - RDS security group done
  - Elasticache security group still in review
  - EKS still in review
- Discussion about `aws-ia/eks-blueprints-addons` Terraform module
  - Will create `aws-ia/eks-blueprints-addon` (without "s") -> will create
    `helm_release` resource
  - Due to being a Helm release, CRD will not be updated along with module
    upgrade. This is Helm limitation (and design?)
  - Required to update CRD through separate job **before** Helm release upgrade

## Other

- Talos bare metal (mostly) unattended provision:
  - Use `nocloud` variant and not `metal` because somehow `metal` couldn't detect
    `metal-iso` partition
  - `dd` the ISO, fix backup partition table and add new partition using `gdisk`
    1. `sudo gdisk <DEVICE>`
    1. `x` (extra functionality)
    1. `e` (relocate backup data structures to the end of the disk)
    1. `m` (return to main menu)
    1. `n` (add a new partition)
    1. TODO
    1. `w` (write table to disk and exit)
  - <https://www.talos.dev/v1.9/talos-guides/install/cloud-platforms/nocloud/>
